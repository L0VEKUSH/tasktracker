...
export const authService = {
  ...
  signup: (userData) => api.post('/api/auth/signup', userData).then((res) => res.data),
  getMe: () => api.get('/api/auth/me').then((res) => res.data),
};

export const taskService = {
  getAll: (params) => api.get('/api/tasks', { params }).then((res) => res.data),
  getStats: () => api.get('/api/tasks/stats').then((res) => res.data),
  getById: (id) => api.get(`/api/tasks/${id}`).then((res) => res.data),
  create: (payload) => api.post('/api/tasks', payload).then((res) => res.data),
  update: (id, payload) => api.put(`/api/tasks/${id}`, payload).then((res) => res.data),
  remove: (id) => api.delete(`/api/tasks/${id}`).then((res) => res.data),
};
